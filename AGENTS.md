...
- Known upstream Thinking Effort bug, verified on June 20, 2026 with VS Code 1.125.1 and bundled Copilot Chat 0.53.1: the Copilot extension-contributed model path receives the per-editor selection in `ChatRequest.modelConfiguration`, converts it to `modelCapabilities.reasoningEffort`, but `extensions/copilot/src/platform/endpoint/vscode-node/extChatEndpoint.ts` drops `modelCapabilities` before its nested `languageModel.sendRequest(...)`. The UI can therefore display `High` while an OAIProxy provider still receives the schema/profile default such as `max`. This is tracked by [microsoft/vscode#319794](https://github.com/microsoft/vscode/issues/319794), with the proposed forwarding fix in [microsoft/vscode#319798](https://github.com/microsoft/vscode/pull/319798). The old `microsoft/vscode-copilot-chat` repository is archived; check `microsoft/vscode` for current status. After any VS Code or Copilot update, recheck the issue, PR, and current `extChatEndpoint.ts`, then verify the actual OAIProxy request log rather than trusting the picker label. The proposed upstream fix forwards the per-request value as `options.modelOptions.reasoningEffort`; OAIProxy should accept both that field and `options.modelConfiguration.reasoningEffort`, with the per-request `modelOptions` value taking precedence over a stale/default `modelConfiguration` value.
