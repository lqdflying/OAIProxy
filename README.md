...
Quick Setup also includes **Grok 4.7 (OAuth)** and **Grok 4.6 (OAuth)**. These cards use xAI's subscription Responses route at `https://cli-chat-proxy.grok.com/v1`; they do not require an API key. Run **OAIProxy: Sign in to xAI / Grok with OAuth** once, complete the device-code approval in a browser, then add either card. OAIProxy stores the refreshable credential in VS Code SecretStorage and refreshes it before requests. Use **OAIProxy: Sign out of xAI / Grok OAuth** to remove it. OAuth availability is controlled by xAI account eligibility; xAI API-key models remain available separately through `https://api.x.ai/v1`.
