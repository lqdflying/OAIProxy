...
`litellm.jsonc` uses the gateway aliases `Kimi-K2.6`, `GLM-5.2`, `GLM-5.3-Flash`, `GLM-5.3`, `DeepSeek-V4.1-Flash`, and `Qwen3.8-27B`. Qwen uses its native 262,144-token context with a conservative 65,536-token output allowance. Removing retired Quick Setup cards does not change existing saved models.
